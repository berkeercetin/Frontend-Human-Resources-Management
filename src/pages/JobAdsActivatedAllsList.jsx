...
export default function JobAdsActivatedAllsList() {
    ...
    const [jobAdsActivateds, setJobAdsActivated] = useState([])

    useEffect(() => {
        let jobAdsActivatedService = new JobAdvertisementsService()
        jobAdsActivatedService.getActivatedJobAdvertisements().then(result =>  setJobAdsActivated(result.data.data))
    }, [])
    return (
        <div>
            <Table fixed>
                ...
                <Table.Body>
                    {
                        jobAdsActivateds.map(jobAdsActivated => (<Table.Row key={jobAdsActivated.id}>
